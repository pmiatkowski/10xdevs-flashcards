  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        // Jeśli nie możemy sparsować JSON, użyjemy domyślnej wiadomości
    ...
    try {
      ...
      console.error("Failed to parse OpenRouter response content:", parseError);
...
function validateAndPrepareCandidates(
  ...
  rawCandidates.forEach((candidate, index) => {
    ...
    if (!front_text || !back_text) {
      console.warn(`Skipping empty candidate at index ${index}`);
      return;
    }
...
export async function generateCandidates(
  ...
  console.log("Source text hash (MD5):", source_text_hash);
  ...
  try {
    ...
    console.log(`Received ${rawCandidates.length} raw candidates from AI.`);
  } catch (error) {
  ...
  let candidatesToInsert: AiCandidateInsert[];
  try {
    candidatesToInsert = validateAndPrepareCandidates(rawCandidates, userId, source_text_hash);
    ...
    // Rzucenie błędu walidacji, aby endpoint zwrócił 400
    throw validationError;
  }

  // Krok 7f: Wstawienie kandydatów do tabeli ai_candidates
  console.log(`Attempting to insert ${candidatesToInsert.length} candidates into DB.`);
  ...
  if (insertError) {
    console.error("Database insert error (ai_candidates):", insertError);
  ...
  if (!insertedCandidates || insertedCandidates.length === 0) {
    ...
    console.error("No candidates returned after successful insert operation.");
    throw new Error("Failed to retrieve saved AI candidates after insert.");
  }

  const candidateCount = insertedCandidates.length;
  console.log(`Successfully inserted ${candidateCount} candidates into DB.`);
  ...
  console.log("Inserting generation stats:", statsToInsert);
  ...
  if (statsError) {
    ...
    console.error("Failed to insert generation stats:", statsError);
